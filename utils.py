...
def init_cuda():
    ...
    debug(
        "PyTorch using {} device {}:{} with Compute Capability {}.{}".format(
            str(device).upper(),
...
class TranslationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        inputs, targets = self.encode_sentences(
    ...
    def encode_sentences(
        self, target_sentences, source_sentences, tokenizer, max_length
    ):
        # Implement the logic to encode sentences here
        pass


# Example dataset (you'll need to replace this with your actual Arabic-English dataset)
source_texts = ["مرحبا", "كيف حالك؟"]
target_texts = ["Hi", "How are you?"]
...
def read_langs(lang1, lang2, reverse=False):
    ...
    print(os.getcwd())
    lines = (
        open("data/{}-{}.txt".format(lang1, lang2), encoding="utf-8")
        .read()
        .strip()
        .split("\n")
    )
...
def prepare_data(lang1, lang2, reverse=False):
    ...
    val_idx = int(0.9 * len(pairs))
    return input_lang, output_lang, pairs[:val_idx], pairs[val_idx:]
...
def tensor_from_sentence(lang, sentence):
    ...
    return torch.tensor(indices, dtype=torch.long, device=torch.device("cuda")).view(
        -1, 1
    )
