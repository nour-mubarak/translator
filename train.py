...
def parse_arguments():
    ...
    parser.add_argument(
        "--lr", type=float, default=0.001, dest="lr", help="Initial learning rate"
    )
    ...
    parser.add_argument(
        "--log-level", type=str, default="info", dest="log_level", help="Logging level"
    )
...
def optimize(
    ...
    for input_tensor, target_tensor in training_tensors:
        ...
        encoder_outputs = torch.zeros(
            max_length, encoder.hidden_size, device=torch.device("cuda")
        )
...
def train_epochs(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % save_every == 0:
            ...
            torch.save(
                encoder.state_dict(),
                "models/{}/{}_{}.encoder".format(model_name, epoch, model_name),
            )
            torch.save(
                decoder.state_dict(),
                "models/{}/{}_{}.decoder".format(model_name, epoch, model_name),
            )
...
def create_models(config, in_words, out_words):
    ...
    encoder = EncoderRNN(
        in_words, int(config["hidden_size"]), num_layers=int(config["num_layers"])
    ).cuda()
...
def main():
    ...
    logging.basicConfig(
        format=LOG_FORMAT, level=getattr(logging, args.log_level.upper())
    )
    ...
    if not os.path.isdir(model_dir):
        ...
        encoder, decoder = create_models(
            config["rnn"], input_lang.n_words, output_lang.n_words
        )

        logging.info("Saving model configuration for evaluation...")
        with open(
            "models/{}/{}.cfg".format(args.model_name, args.model_name), "w"
        ) as config_file:
        ...
        try:
            ...
            config_file = "models/{}/{}".format(
                args.model_name, list(filter(lambda x: ".cfg" in x, files))[0]
            )
            config.read(config_file)

            logging.info("Loading data...")
            data_file = "models/{}/{}".format(
                args.model_name, list(filter(lambda x: ".data.pkl" in x, files))[0][:-4]
            )
            ...
            iters = [
                int(x.split("_")[0])
                for x in files
                if ".encoder" in x or ".decoder" in x
            ]
            max_iter = str(max(iters))

            encoder_path = "models/{}/{}".format(
                args.model_name,
                list(
                    filter(lambda x: x.startswith(max_iter) and ".encoder" in x, files)
                )[0],
            )
            decoder_path = "models/{}/{}".format(
                args.model_name,
                list(
                    filter(lambda x: x.startswith(max_iter) and ".decoder" in x, files)
                )[0],
            )
    ...
    torch.save(
        encoder.state_dict(),
        "models/{}/{}.encoder".format(args.model_name, args.model_name),
    )
    torch.save(
        decoder.state_dict(),
        "models/{}/{}.decoder".format(args.model_name, args.model_name),
    )
